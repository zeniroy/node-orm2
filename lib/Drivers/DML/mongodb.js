...
Driver.prototype.sync = function (opts, cb) {
	this.db.createCollection(opts.table, function (err, collection) {
		...
		var indexes = [], pending, unique_indexes = [];
		...
		for (k in opts.properties) {
			if (opts.properties[k].unique) {
				unique_indexes.push(k);
			} else if (opts.properties[k].index) {
				indexes.push(k);
			}
		}

		pending = indexes.length + unique_indexes.length;
		...
		for (i = 0; i < unique_indexes.length; i++) {
			collection.createIndex(unique_indexes[i], {unique:true}, function () {
				if (--pending === 0) {
					return cb();
				}
			});
...
Driver.prototype.connect = function (cb) {
    if(!this.config.href) {
		this.config.href = this.config.protocol + "://" + this.config.host + ":" + this.config.port + "/" + this.config.database;
	}  
	...
	this.client.connect(this.config.href, function (err, db) {
		...
		this.db = db;
		if(this.config.user) {
			this.db.authenticate(this.config.user, this.config.password, function(err, res){
				return cb();
			});
		}
